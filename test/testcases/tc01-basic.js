...
export class UserDao {
	...
	static harvestData(row: {[name: string]: any}, dest?: any) {
		...
		else if (_.isDate(row.name)) dest.name = row.name.toISOString();
		...
		else if (_.isDate(row.addr)) dest.addr = row.addr.toISOString();
		...
		if (_.isString(row.desc)) dest.desc = row.desc;
		else if (_.isDate(row.desc)) dest.desc = row.desc.toISOString();
		else if (row.desc === null || row.desc === undefined) dest.desc = null;
		else throw new TypeError('Wrong type for row.desc');
	...
	static async query(sql: string, params: any[], conn: Pick<Connection, 'execute'>, options?: {log?: LogFunction}): Promise<User[]>;
	static async query(sql: string, arg1: any, arg2: any, arg3?: any): Promise<User[]> {
	...
	static async create(data: UserData, conn: Pick<Connection, 'execute'>, options?: {log?: LogFunction}): Promise<User> {
		...
		if (data.desc === null || data.desc === undefined) params.desc = null;
		else params.desc = data.desc;
